...
from utils import print_message, print_error, print_menu
...
def initConcert():
    while True:
        print_message("Enter concert duration in this format: hours minutes seconds")
        try:
...
def operateConcert(concert):
    while True:
        ...
        if choice == 0:
            ...
            concert.pushAttendee()
        elif choice == 2:
            concert.popAttendee()
        elif choice == 3:
            concert.peekLastAttendee()
        elif choice == 4:
            concert.getAttendanceDuration()
        elif choice == 5:
            concert.detectRuleViolations()
        elif choice == 6:
            concert.displayAttendees()
        elif choice == 7:
            concert.generateAttendanceReport()
        else:
            print_error(f"{choice} is not a valid input!")
